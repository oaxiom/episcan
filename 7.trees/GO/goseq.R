...
setwd("/Users/andrew/Projects/Episcan/episcan/7.trees/GO")

filenames = list.files(path="repacked/", pattern="*.tsv$")

#all = read.delim("all_genes.tsv")$ensg
all = read.delim("~/hg38/hg38_ensembl_v95_ensg.tsv")$ensg

for (filename in filenames) {
  print(filename)
  
  data = read.delim(paste("repacked/", filename, sep=""), row.names=1) # id for deseq
  ensg = rownames(data)
