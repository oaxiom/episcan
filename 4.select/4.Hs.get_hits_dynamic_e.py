...
epifactors = set(epifactors.removeDuplicates('name')['name'])
...
print('Episcan ( {0} ( {1} ) {2} ) Epifactors'.format(len(episcan_only), len(both), len(epifactors_only)))

with open('result_epifactors_only.txt', 'wt') as oh:
    for name in sorted(epifactors_only):
        oh.write('{0}\n'.format(name))

with open('result_both.txt', 'wt') as oh:
    for name in sorted(both):
        oh.write('{0}\n'.format(name))

with open('result_episcan_only.txt', 'wt') as oh:
